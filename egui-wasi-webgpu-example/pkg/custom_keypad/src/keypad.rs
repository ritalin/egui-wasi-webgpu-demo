...
impl KeyState {
    fn new() -> Self {
        Self {
            ...
            start_pos: egui::pos2(100.0, 100.0),
            focus: None,
            events: None,
        }
    }
...
impl Keypad {
    ...
    pub fn bump_events(&self, ctx: &egui::Context, raw_input: &mut egui::RawInput) {
        let events = ctx.memory_mut(|m| {
            m.data
                .get_temp_mut_or_default::<KeyState>(self.id)
                .events
                .take()
        });
    ...
    fn buttons(ui: &mut egui::Ui, state: &mut KeyState) -> Transition {
        let mut trans = Transition::None;
        ui.vertical(|ui| {
            let window_margin = ui.spacing().window_margin;
            let size_1x1 = egui::vec2(32.0, 26.0);
            let _size_1x2 = egui::vec2(32.0, 52.0 + window_margin.topf());
            let _size_2x1 = egui::vec2(64.0 + window_margin.leftf(), 26.0);

            ui.spacing_mut().item_spacing = egui::Vec2::splat(window_margin.leftf());

            ui.horizontal(|ui| {
                if ui.add_sized(size_1x1, egui::Button::new("1")).clicked() {
                    state.queue_char('1');
                }
                if ui.add_sized(size_1x1, egui::Button::new("2")).clicked() {
                    state.queue_char('2');
                }
                if ui.add_sized(size_1x1, egui::Button::new("3")).clicked() {
                    state.queue_char('3');
                }
                if ui.add_sized(size_1x1, egui::Button::new("⏮")).clicked() {
            ...
            ui.horizontal(|ui| {
                if ui.add_sized(size_1x1, egui::Button::new("4")).clicked() {
                    state.queue_char('4');
                }
                if ui.add_sized(size_1x1, egui::Button::new("5")).clicked() {
                    state.queue_char('5');
                }
                if ui.add_sized(size_1x1, egui::Button::new("6")).clicked() {
                    state.queue_char('6');
                }
                if ui.add_sized(size_1x1, egui::Button::new("⏭")).clicked() {
            ...
            ui.horizontal(|ui| {
                if ui.add_sized(size_1x1, egui::Button::new("7")).clicked() {
                    state.queue_char('7');
                }
                if ui.add_sized(size_1x1, egui::Button::new("8")).clicked() {
                    state.queue_char('8');
                }
                if ui.add_sized(size_1x1, egui::Button::new("9")).clicked() {
                    state.queue_char('9');
                }
                if ui.add_sized(size_1x1, egui::Button::new("⏶")).clicked() {
            ...
            ui.horizontal(|ui| {
                if ui.add_sized(size_1x1, egui::Button::new("0")).clicked() {
                    state.queue_char('0');
                }
                if ui.add_sized(size_1x1, egui::Button::new(".")).clicked() {
                    state.queue_char('.');
    ...
    pub fn show(&self, ctx: &egui::Context) {
        let (focus, mut state) = ctx.memory(|m| {
            (
                m.focused(),
                m.data.get_temp::<KeyState>(self.id).unwrap_or_default(),
            )
        });
        ...
        if is_first_show {
            ...
            state.start_pos = ctx.input(|i| {
                i.pointer
                    .hover_pos()
                    .map_or(egui::pos2(100.0, 100.0), |p| p + egui::vec2(0.0, y))
            });
            state.focus = focus;
        }
