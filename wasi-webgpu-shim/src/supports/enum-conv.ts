import type { GpuCanvasAlphaMode, GpuTextureFormat, GpuTextureViewDimension } from "../types/wasi-webgpu-webgpu.js";
...
const TEXTURE_FORMAT_MAP_REV = Object.fromEntries(Object.entries(TEXTURE_FORMAT_MAP).map(([k, v]) => [v, k])) as Record<
  GPUTextureFormat,
  GpuTextureFormat
>;
...
export const TEXTURE_VIEW_DIMENSION_MAP: Record<GpuTextureViewDimension, GPUTextureViewDimension> = {
  d1: "1d",
  d2: "2d",
  "d2-array": "2d-array",
  cube: "cube",
...
const CANVAS_ALPHA_MODE_MAP: Record<GpuCanvasAlphaMode, GPUCanvasAlphaMode> = {
  opaque: "opaque",
  premultiplied: "premultiplied",
};

const CANVAS_ALPHA_MODE_MAP_REV = Object.fromEntries(
  Object.entries(CANVAS_ALPHA_MODE_MAP).map(([k, v]) => [v, k]),
) as Record<GPUCanvasAlphaMode, GpuCanvasAlphaMode>;

export const CanvasAlphaMode = {
  fromWasi: (v0: keyof typeof CANVAS_ALPHA_MODE_MAP) => {
    const v = CANVAS_ALPHA_MODE_MAP[v0];
    if (v === undefined) {
      throw new Error(`Shim: Missing mapping for "${v0}"`);
    }
    return v;
  },
  intoWasi: (v0: keyof typeof CANVAS_ALPHA_MODE_MAP_REV) => {
    const v = CANVAS_ALPHA_MODE_MAP_REV[v0];
    if (v === undefined) {
      throw new Error(`Shim: Missing mapping for "${v0}"`);
    }
    return v;
